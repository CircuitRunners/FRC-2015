...
public class Forklift extends Subsystem {
    ...
    public static void lift(double speed) {
        if ((limitSensorBot.get() && speed < 0 || limitSensorTop.get() && speed > 0) && SmartDashboard.getBoolean("DB/Button 0")) {
            speed = 0;
        }
        liftMotor.set(speed);
    }
    ...
    public static void fork(double speed) {
        if (!limitSensorFork.get() && speed < 0 && SmartDashboard.getBoolean("DB/Button 1")) {
            speed = 0;
        }
        forkMotor.set(speed);
    }
